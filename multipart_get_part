...
is given by the script's single (zero-indexed) integer argument.
...
parser = argparse.ArgumentParser(
    description="Replace a multipart message by one of its child subparts."
)
parser.add_argument("part", type=int, help="zero-indexed subpart number to select")
...
subpart = None
for k, part in enumerate(msg.iter_parts()):
    if k == selected:
        subpart = part
        break
if subpart is None:
    raise ValueError(f"Message does not have a subpart {selected}.")
